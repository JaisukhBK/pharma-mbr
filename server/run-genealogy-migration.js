// run-genealogy-migration.js — Run from server folder: node run-genealogy-migration.js
...
const TABLES = [
  {
    ...
    sql: `CREATE TABLE IF NOT EXISTS material_master (
  ...
  {
    name: 'material_lots',
    sql: `CREATE TABLE IF NOT EXISTS material_lots (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  ...
  },
  {
    ...
    sql: `CREATE TABLE IF NOT EXISTS material_transactions (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  ...
  },
  {
    ...
    sql: `CREATE TABLE IF NOT EXISTS batch_genealogy (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      ebr_id UUID,
...
async function run() {
  ...
  for (const table of TABLES) {
    try {
      ...
      if (e.message.includes('already exists')) {
        ...
        console.error(`✗ Table: ${table.name} — ${e.message}`);
      }
    }
  }
  ...
  for (const idx of INDEXES) {
    try {
      ...
      console.log(`⊘ Index: ${e.message.substring(0, 60)}`);
    }
  }
  ...
  console.log('\n=== VERIFICATION ===');
  for (const table of TABLES) {
    const r = await client.query(`SELECT COUNT(*) as cnt FROM ${table.name}`).catch(() => ({ rows: [{ cnt: 'MISSING' }] }));
    console.log(`  ${table.name}: ${r.rows[0].cnt} rows`);
  }
