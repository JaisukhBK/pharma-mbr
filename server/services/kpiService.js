...
async function getKPIs() {
  const [batches, mbrs, devs, training, equipment] = await Promise.all([
    query("SELECT COUNT(*) as total, COUNT(*) FILTER (WHERE status='In Progress') as in_progress, COUNT(*) FILTER (WHERE status='Released') as released, COUNT(*) FILTER (WHERE status='Complete') as completed FROM ebrs"),
  ...
  return {
    batches: { total: parseInt(b.total), in_progress: parseInt(b.in_progress), released: parseInt(b.released), completed: parseInt(b.completed), avg_yield: null },
...
function calculateOEE(batchData) {
  const total = parseInt(batchData.total) || 1;
  const released = parseInt(batchData.released);
  const availability = 0.92;
  const performance = Math.min(1, total > 0 ? (released + parseInt(batchData.completed)) / total : 0);
  const quality = 0.97;
  return { availability: (availability * 100).toFixed(1), performance: (performance * 100).toFixed(1), quality: (quality * 100).toFixed(1), oee: (availability * performance * quality * 100).toFixed(1) };
}

async function getBatchSummary(filters = {}) {
  let sql = `SELECT e.id, e.ebr_code as batch_id, e.batch_number, e.product_name, e.status, e.started_at, e.completed_at,
...
async function getProducts() {
  const r = await query("SELECT DISTINCT product_name, product_code FROM mbrs WHERE product_name IS NOT NULL ORDER BY product_name");
  return r.rows;
}
